...
def generate_random_correspondences(num_points: int,
                                    im_size: Tuple[int, int],
                                    min_val_3d: Tuple[float, float, float],
                                    max_val_3d: Tuple[float, float, float],
                                    ) -> Tuple[np.ndarray, np.ndarray]:
    """Generate 2D-3D correspondences randomly"""

    random_2d_points = np.random.uniform(
        low=[0.0, 0.0], high=[im_size[1], im_size[0]], size=(num_points, 2))

    random_3d_points = np.random.uniform(
        low=min_val_3d, high=max_val_3d, size=(num_points, 3))

    return random_2d_points, random_3d_points
